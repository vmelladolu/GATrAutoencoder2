...
def test_forward_on_gpu(model, cfg_enc, device):
    ...
    try:
        ...
        wprint("Grafico guardado en gatr_autoencoder_graph.png")
    except Exception:
        wprint("No se pudo dibujar la red (torchviz no disponible).")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Training script for GATrRegressor (PyTorch Lightning)")
    ...
    parser.add_argument("--save-every", type=int, default=500, help="Save checkpoint every N train steps (0 disable)")
    parser.add_argument("-o", "--out", type=str, default="./trained_model_2")
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Explicit GPU ids to use, e.g. --gpu_ids 1 2 3",
    )
    parser.add_argument("--devices", type=int, default=4, help="Number of devices (default: 4 GPUs)")
    parser.add_argument("--accelerator", type=str, default="gpu", help="Trainer accelerator (gpu/cpu/auto)")
    parser.add_argument("--strategy", type=str, default="ddp", help="Trainer strategy (ddp/auto)")
    parser.add_argument("--train-num-workers", type=int, default=4, help="Train dataloader workers")
    parser.add_argument("--val-num-workers", type=int, default=2, help="Val dataloader workers")
    parser.add_argument("--lr", type=float, default=1e-3, help="Learning rate")
    parser.add_argument("--seed", type=int, default=42, help="Seed used for split and trainer")
...
def main():
    ...
    if cfg_dec["out_s_channels"] != cfg_enc["in_s_channels"]:
        wprint(
            ...
            f"{cfg_dec['out_s_channels']} to {cfg_enc['in_s_channels']}"
        )
    ...
    base_model = GATrRegressor(cfg_enc=cfg_enc, cfg_agg=cfg_agg)
    if args.test:
    ...
    callbacks = []
    # Callback para registrar learning rate en wandb
    callbacks.append(LearningRateMonitor(logging_interval="step"))
    if args.save_every > 0:
        callbacks.append(
            ModelCheckpoint(
                dirpath=checkpoint_path,
                filename="gatr_regressor_epoch{epoch:02d}_step{step}",
    ...
    callbacks.append(
        ModelCheckpoint(
            dirpath=checkpoint_path,
            filename="gatr_regressor_best_epoch{epoch:02d}_lossval{loss/val:.6f}",
            monitor="loss/val",
            mode="min",
            save_top_k=2,
            save_weights_only=False,
