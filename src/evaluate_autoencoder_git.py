...
def run_inference(model, loaders_with_labels, device, use_scalar, use_one_hot):
    ...
    for loader, label_int, label_name in loaders_with_labels:
        ...
        for batch in tqdm(loader):
            ...
            with torch.no_grad():
                if bidx.numel() == 0:
                    print("[WARNING] bidx vacío")
                    continue

                B = int(bidx.max().item()) + 1
                num_nodes = mv_v.shape[0]

                if B <= 1 or num_nodes <= 1:
                    print(f"[WARNING] Batch degenerado skippeado: B={B}, nodes={num_nodes}")
                    print("mv_v shape:", mv_v.shape)
                    print("mv_s shape:", mv_s.shape)
                    print("sc shape:", sc.shape)
                    print("bidx:", bidx.unique())
                    continue

                if torch.isnan(mv_v).any() or torch.isinf(mv_v).any():
                    print("[WARNING] NaN/Inf en mv_v")
                    continue

                if torch.isnan(sc).any() or torch.isinf(sc).any():
                    print("[WARNING] NaN/Inf en sc")
                    continue
            ...
            if hasattr(batch, "energy") and batch.energy is not None:
                energies_batch = batch.energy.cpu().numpy()
                #print("B =", B)
               #print("energy shape =", energies_batch.shape)
            else:
