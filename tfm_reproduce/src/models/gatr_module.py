...
class GATrBasicModule(nn.Module):
    ...
    def build_asymmetric_query_mask(self, batch, n_ctx_per_event: int):
        """Asymmetric attention mask for query-token AR decoding.

        Context tokens (seed + generated hits) cannot attend to query tokens.
        Query tokens can attend to all tokens within their event.

        Args:
            batch: (N,) event indices. N = n_events * (n_ctx_per_event + n_query).
            n_ctx_per_event: number of context tokens (seed + generated) per event.
                             Must be identical for all events in the batch.

        Returns:
            (1, 1, N, N) float tensor with 0.0 (allow) or -inf (block).
            Passed as additive attention bias to GATr / PyTorch SDPA.
        """
        N = len(batch)
        mask = torch.zeros(N, N, device=batch.device, dtype=torch.float32)
        seqlens = torch.bincount(batch.long()).tolist()
        offset = 0
        for L in seqlens:
            # rows = context tokens, cols = query token columns
            n_q = L - n_ctx_per_event
            if n_q > 0:
                mask[offset:offset + n_ctx_per_event,
                     offset + n_ctx_per_event:offset + L] = float('-inf')
            offset += L
        return mask.unsqueeze(0).unsqueeze(0)   # (1, 1, N, N)

    # ---------------------------------------------
    def forward(self, mv_v_part, mv_s_part, scalars, batch,
                embedded_geom=None, n_ctx_per_event=None):
        ...
        if n_ctx_per_event is not None:
            mask = self.build_asymmetric_query_mask(batch, n_ctx_per_event)
        else:
