...
class SDHCALRegressorDataModule(L.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        print(
            f"[DataModule] Split: {len(self.train_dataset)} train / "
            f"{len(self.val_dataset)} val eventos"
        )
...
def _apply_filters_inmem(event_data: dict, n_events: int, filters: dict) -> np.ndarray:
    ...
    Each value in ``filters`` may be:
    - a single string condition:  ``">=50"``
    - a list of string conditions (all must pass): ``[">=1", "<=120"]``
    - an exact-match scalar or string
    """
    if not filters:
    ...
    for filter_key, filter_value in filters.items():
        ...
        # Support list of conditions for range filters (e.g. [">=1", "<=120"])
        conditions = filter_value if isinstance(filter_value, list) else [filter_value]
        for cond in conditions:
            if isinstance(cond, str) and cond.startswith((">=", "<=", ">", "<")):
                op, num = _parse_filter_operator(cond)
                if op == ">=":
                    mask &= arr >= num
                elif op == "<=":
                    mask &= arr <= num
                ...
                compare_value = cond
                if filter_key in STRING_TO_INT_MAPPINGS and isinstance(cond, str):
                    mapping = STRING_TO_INT_MAPPINGS[filter_key]
                    compare_value = mapping.get(cond, cond)
